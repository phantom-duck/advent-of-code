...
if __name__=="__main__":
	...
	robot_position = (0, 0)
	robot_facing = 0
	robot.io.give_input(1)
	ret = robot.run()
	while ret != 0:
		colour = robot.io.get_output()
		new_direction = robot.io.get_output()
		
		panel_grid[robot_position] = colour
		if new_direction == 0:
			robot_facing = (robot_facing + 3) % 4
		else:
			robot_facing = (robot_facing + 1) % 4
		if robot_facing % 2 == 0:
			robot_position = (robot_position[0] + (robot_facing - 1), robot_position[1])
		else:
			robot_position = (robot_position[0], robot_position[1] - (robot_facing - 2))
		
		if robot_position in panel_grid:
			robot.io.give_input(panel_grid[robot_position])
		else:
			robot.io.give_input(0)
		ret = robot.run()
	
	painting = [41 * [0] for i in range(6)]
	for i in range(6):
		for j in range(41):
			if (i, j) in panel_grid:
				painting[i][j] = panel_grid[(i, j)]
	
	painting = '\n'.join(list(''.join(str(num) for num in line) for line in painting))
